/**
 * This is a simple GPU accelerated ai program.
 * It uses cuda programming to offload work to the GPU.
 * This won't work unless you have an nvidia gpu (it needs cuda cores)
 * 
 * The program assigns work to the GPU. The GPU looks through the moves that could be made, puts them in an array
 * then sorts them.  When the from the GPU are brought back here, the arrays are processed and merged, and the best move is played from all the options
 * generated by the gpu is given.
 */

// This is literally the most braindead way to program an AI.  I could have used something like Alpha-beta pruning, but I'm too lazy and I want
// immediate results.  Also, I want to use CUDA programming and I don't understand how to paralellize the more complicated methods for
// chess AI for a GPU.  Basically all I am going to do is just go through positions and evaluate them, then sort them by how good they are.
// I just did some basic math and I think this should be able to look at least 5-6 moves into the future, which is much better than me.
